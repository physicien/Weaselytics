...
> **The calibration set no longer exists.** `FCUT_GALLERY_2026-07-20` and `FCUT_SELECT_2026-07-21/fcut_dataset.csv` — the 348 hand-labeled ranges every constant below comes from — were deleted on 2026-07-22, together with the `channel_curves/` comparison. Everything in this section is therefore a **historical record of a measurement that can no longer be re-run**: `tools/fcut_bracket_calib.py` has no input, and the agreement check it performs cannot be repeated. The constants shipped in `refine_candidates` are unchanged and were correct for the data they were fitted on, but they are now unauditable, and they were in any case calibrated against candidate regions produced by two superseded code paths (see the third caveat below). Treat them as placeholders pending a new labeling session, not as validated values.
