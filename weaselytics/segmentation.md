...
## 4c. Narrowing the candidates with the labeled bracket (REMOVED — historical, unverifiable)

> **REMOVED 2026-07-26. Kept only as a record of a route that should not be taken again.** `refine_candidates` and `tools/fcut_bracket_calib.py` were deleted from the package.
>
> **Why the route was ill-advised.** The bracket narrowed the candidate regions using constants fitted to a hand-labeled gallery — but that gallery sampled cutoffs *only inside* those same candidate regions and drew them on the figures the labeller looked at. Labels collected that way are confined to, and visually anchored on, the machinery they are then used to calibrate, so they cannot test its boundaries and the agreement it reports is partly circular. The approach also made three shipped constants hostage to a single labeling session on a single instrument: nothing else could ever confirm them, and there was no path to generalising them to another column or another dataset.
>
> That fragility is now realised. The calibration set (`FCUT_GALLERY_2026-07-20`, 348 ranges) was deleted 2026-07-22 and its replacement (`FCUT_GALLERY_2026-07-22`, 51 labels) on 2026-07-26 once those labels were judged untrustworthy. No label set of any kind remains, so the constants can never be re-derived, re-validated, or checked for having been reported correctly.
>
> **Every figure below is therefore unverifiable — the spans, the separations, the recall and the leave-one-out numbers alike. Do not cite them, do not build on them, and do not treat them as measurements.** This section should itself be re-read and probably deleted once someone has judged whether it records anything worth keeping. See the README TO DO.
...
Three call sites shared an unguarded `np.where(...)[0][-1]` or `[0]` on a possibly-empty array. They now raise a `ValueError` naming the tolerance, the window and the `freq_cutoff=` bypass, instead of an opaque `IndexError`. Nothing substitutes a cutoff: a wrong `fcut` silently biases every area derived from it, which is worse than no answer. The exception at the time was `find_plateaus`, whose four outputs only fed the diagnostic overlay in `r2_plots` and never reached `fcut` — a failure there disabled the overlay rather than aborting the selection, which is what recovered the 67th signal. *(Historical: the derivative route, its tolerances and `find_plateaus` itself have since been removed — the overlay is now driven by `dip_curve`. The reasoning about failing loudly rather than substituting a cutoff still applies to the surviving call sites.)*
