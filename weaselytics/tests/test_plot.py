...
class TestR2Plots:
    def test_r2_plots_basic(self):
        ...
        plateau_mask = np.zeros(100, dtype=bool)
        plateau_mask[:20] = True
        r2_plots(x, r2, rolling_std, diff_std_mad, plateau_mask, 0.01, 0.5)
    ...
    def test_r2_plots_with_changepoint_overlay(self, tmp_path):
        ...
        cp_flat[20:60] = True
        r2_plots(x, r2, np.gradient(r2), np.gradient(np.gradient(r2)),
                 np.zeros(100, dtype=bool), 0.01, 0.95,
                 cp_flat=cp_flat,
